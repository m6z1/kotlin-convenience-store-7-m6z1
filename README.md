...
### 구매자의 `할인 혜택`과 `재고 상황`을 고려하여 `최종 결제 금액을 계산하고 안내`하는 결제 시스템
...
- [X] 각 프로모션에 대한 정보를 가진다.
- [X] 오늘 날짜가 프로모션 기간 내인지 확인 후 할인을 적용한다.
...
- [X] 멤버십 회원은 프로모션 미적용 총 금액의 30%를 할인받는다.
- [X] 프로모션 적용 시엔 프로모션 적용 후 남은 금액에 대해 멤버십 할인 적용한다.
- [X] 멤버십 할인 최대 한도는 8,000원이다.

### 영수증

- [X] 영수증 항목은 아래의 값들을 가진다.
